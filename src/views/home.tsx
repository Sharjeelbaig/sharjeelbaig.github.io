...
`);
  ...
  const handleMessageSubmit = (e: any) => {
    ...
    if (
      ...
      setEmailStatus({ ...emailStatus, empty: true });
      return;
    }
    emailjs
      .sendForm(
      ...
      .then(
        () => {
          setEmailStatus({ ...emailStatus, success: true });
        },
        () => {
  ...
  useEffect(() => {
    if (emailStatus.success || emailStatus.error || emailStatus.empty) {
      setTimeout(() => {
        setEmailStatus({
          ...emailStatus,
          success: false,
  ...
  return (
    <div>
      ...
      <Snackbar
        ...
        open={emailStatus.success}
        sx={{ backgroundColor: "purple", color: "white" }}
        message="Email sent successfully"
      />
      <Snackbar
        anchorOrigin={{ vertical: "top", horizontal: "left" }}
        open={emailStatus.error}
        sx={{ backgroundColor: "red", color: "white" }}
        message="Error sending email"
      />
      <Snackbar
        anchorOrigin={{ vertical: "top", horizontal: "left" }}
        open={emailStatus.empty}
        sx={{ backgroundColor: "red", color: "white" }}
      ...
      <Box textAlign="center">
        ...
        <Box p={2} id="home" mt={8}>
          ...
          <Box sx={{ mt: 5 }}>
            ...
            <Typography variant="subtitle1" sx={styles.normal}>
              {basic.designation}{" "}
            </Typography>
          </Box>
        </Box>
        ...
        {/* Contact Information */}
        <Box p={2}>
          <Typography variant="h5" sx={styles.heading} gutterBottom>
            Contact Information
          </Typography>
            ...
            {Object.entries(contact).map(([key, value]) => (
              <Grid key={key} item xs={12} sm={6} md={4} lg={4}>
                <Paper
                  elevation={3}
                  variant="outlined"
                  sx={{ p: 2, textAlign: "center", height: "6rem" }}
                >
                  {key === "address" && <Place />}
                  {key === "gmail" && <EmailIcon />}
                  ...
                  {key === "phone" && <PhoneIcon />}
                  {key === "twitter/X" && <X />}
                  <Typography variant="subtitle1" gutterBottom sx={styles.normal}>
        ...
        >
            ...
            {description.skills.map((skill, index) => (
              <Grid key={index} item xs={6} sm={4} md={4} lg={3}>
                <Paper
                  elevation={3}
                  variant="outlined"
                  sx={{ p: 2, textAlign: "center", borderRadius: "20px" }}
                >
                  <img
                    src={skill.logo}
                    alt={skill.name}
          ...
          <Accordion
            expanded={showMoreSkills}
            onChange={() => setShowMoreSkills(!showMoreSkills)}
            sx={{ position: "unset", boxShadow: "none", my: 5, width: "100%" }}
          >
          ...
          <Grid container spacing={3} justifyContent="center" alignItems="center">
            <Grid item xs={12} sm={6}>
              ...
              >
                ...
                <Typography variant="body2" sx={styles.normal}>
                  {background.education.institution}
                </Typography>
                <Typography variant="body2">
                  {background.education.period}
                </Typography>
              </Paper>
            </Grid>
            <Grid item xs={12} sm={6}>
              ...
              >
                <img
                  src={background.experience.logo}
                  alt="Experience"
                  style={{ width: "100px", height: "100px" }}
                />
                <Typography variant="subtitle1" sx={styles.heading}>
                  {background.experience.designation}
                </Typography>
                <Typography variant="subtitle1" sx={styles.normal}>
                  {background.experience.company}
                </Typography>
                <Typography variant="body2">
                  {background.experience.period}
                </Typography>
              </Paper>
            </Grid>
          </Grid>
            ...
            {projects.map((project, index) => (
              <Grid key={index} item xs={12} sm={6} md={4} lg={4}>
                ...
                >
                  ...
                  <Typography variant="body2" sx={{ textAlign: "left" }} paragraph>
                    {project.description}
                  </Typography>
                  <Box display="flex" justifyContent="space-between">
                    {project["demonstration-link"] && (
                      <Button
                        variant="contained"
                        color="primary"
                        href={project["demonstration-link"]}
                        target="_blank"
                        rel="noopener"
                      >
                        Demo
        ...
        >
          ...
          <Typography variant="body1" gutterBottom>
            For complete details download my resume {}
          </Typography>
        ...
        >
          <Typography
            variant="h5"
            sx={[styles.heading, { color: "black", fontSize: 30 }]}
          >
          ...
          >
            ...
            >
              <em>
                Need a developer? Want to contribute to projects? Want to hire me for
                any role?
              </em>
              <br />
              Feel free to contact me using the form below or by sending an email to{" "}
